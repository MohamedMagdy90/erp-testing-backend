...
function initializeDatabase() {
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_bugs_status ON bugs(status)`, (err) => {
    if (err && !err.message.includes('already exists')) console.error('Error creating bugs status index:', err);
  });

  db.run(`CREATE INDEX IF NOT EXISTS idx_bugs_assignee ON bugs(assignee_id)`, (err) => {
    if (err && !err.message.includes('already exists')) console.error('Error creating bugs assignee index:', err);
  });

  db.run(`CREATE INDEX IF NOT EXISTS idx_bugs_priority ON bugs(priority)`, (err) => {
    if (err && !err.message.includes('already exists')) console.error('Error creating bugs priority index:', err);
  });

  db.run(`CREATE INDEX IF NOT EXISTS idx_bugs_module ON bugs(module_id)`, (err) => {
    if (err && !err.message.includes('already exists')) console.error('Error creating bugs module index:', err);
  });

  db.run(`CREATE INDEX IF NOT EXISTS idx_bug_comments_bug_id ON bug_comments(bug_id)`, (err) => {
    if (err && !err.message.includes('already exists')) console.error('Error creating bug_comments index:', err);
  });
